...
#define dqddk2dxd(dqddk,r,c) (r < NUM_POS ? (r + NUM_POS == c ? 1.0 : 0.0) : dqddk)
	...
	void _integratorGradient(T *ABk, T *s_x, T *s_u, T *s_qdd, T *s_dqdd, T *d_I, T *d_Tbody, T dt, int ld_AB){
		#ifdef __CUDA_ARCH__
			__shared__ T s_xm[STATE_SIZE];	__shared__ T s_qdd2[NUM_POS];	__shared__ T s_dqdd2[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
		#else
			T s_xm[STATE_SIZE];		T s_qdd2[NUM_POS];		T s_dqdd2[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
		#endif
		int start, delta; singleLoopVals(&start,&delta);
		int starty, dy, startx, dx; doubleLoopVals(&starty,&dy,&startx,&dx);
	...
	void _integrator(T *s_xkp1, T *s_x, T *s_u, T *s_qdd1, T *d_I, T *d_Tbody, T dt, T *s_eePos = nullptr){
		...
		#else
			T s_x2[STATE_SIZE];	T s_x3[STATE_SIZE];	T s_qdd2[NUM_POS];	T s_qdd3[NUM_POS];
		#endif
		int start, delta; singleLoopVals(&start,&delta);
		// first compute dynamics at the initial point
	...
	void _integratorGradient(T *ABk, T *s_x, T *s_u, T *s_qdd_1, T *s_dqdd_1, T *d_I, T *d_Tbody, T dt, int ld_AB){
		...
		int starty, dy, startx, dx; doubleLoopVals(&starty,&dy,&startx,&dx);
		#ifdef __CUDA_ARCH__
			__shared__ T s_TMP_1[DIM_AB_r*DIM_AB_c];		__shared__ T s_xm_1[STATE_SIZE];
			__shared__ T s_TMP_2[DIM_AB_r*DIM_AB_c];		__shared__ T s_xm_2[STATE_SIZE];
			__shared__ T s_qdd_2[NUM_POS];					__shared__ T s_dqdd_2[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
			__shared__ T s_qdd_3[NUM_POS];					__shared__ T s_dqdd_3[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
		#else
			 T s_TMP_1[DIM_AB_r*DIM_AB_c];		 T s_xm_1[STATE_SIZE];
			 T s_TMP_2[DIM_AB_r*DIM_AB_c];		 T s_xm_2[STATE_SIZE];
			 T s_qdd_2[NUM_POS];				 T s_dqdd_2[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
			 T s_qdd_3[NUM_POS];			     T s_dqdd_3[NUM_POS*(STATE_SIZE+CONTROL_SIZE)];
		#endif
