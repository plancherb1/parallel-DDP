...
void loadTraj(trajVars<T> *tvars, matDimms *dimms, T *xInit = nullptr, T *uInit = nullptr){
	...
	for (int k=0; k<NUM_TIME_STEPS; k++){
		for (int i = 0; i < STATE_SIZE; i++){
			...
			if (i < CONTROL_SIZE){uk[i] = (uInit != nullptr) ? uInit[i] : 0.0;}
		}
...
template <typename T>
__host__
void loadTraj(CPUVars<T> *cvars, trajVars<T> *tvars, matDimms *dimms, T *xInit = nullptr, T *uInit = nullptr){
...
template <typename T>
__host__
int runOtherOptions(char **argv, T *xInit){
	char mode = argv[1][0];	
	// run the simulator
